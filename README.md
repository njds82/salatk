...
- **دعم كامل للعمل بدون اتصال (Offline-First)**: إمكانية تصفح وتسجيل الصلوات، العادات، المهام وغيرها في حال غياب الإنترنت، مع واجهة تنبيه خاصة بالوضع دون اتصال، ومزامنة تلقائية عند العودة للشبكة.
...
### التخزين المحلي والعمل دون اتصال (Offline-First)

يعتمد التطبيق الآن على **IndexedDB** كذاكرة عميقة لتوفير تجربة استخدام مستمرة حتى عند انقطاع الإنترنت، وذلك من خلال `OfflineStore`:

- **مزامنة ذكية**: عند نجاح جلب البيانات من السحابة، يتم حفظ نسخة في `IndexedDB`.
- **الاسترجاع الاحتياطي (Fallback)**: عند انقطاع الاتصال، تقوم جميع الـ Services (كالصلوات، العادات، المهام) بعرض آخر نسخة محفوظة.
- **التحديث المباشر (Optimistic UI)**: يتم تسجيل العمليات (مثل تأدية الصلاة) محلياً وإظهار النتيجة للمستخدم فوراً قبل المزامنة مع السحابة.
- **تخزين الإعدادات والموقع**: يستخدم `localStorage` لحفظ الإعدادات المفضلة والموقع الجغرافي لحساب أوقات الصلاة دون اتصال.

### الكاش والأداء

- `sw.js` يطبّق تخزيناً مؤقتاً لملفات الواجهة والـ PWA assets
- **تخطي طلبات Supabase**: يتم تمرير جميع طلبات `Supabase` مباشرة للمتصفح دون اعتراض من الـ `Service Worker`، وذلك لتجنب أخطاء الـ `CORS` ومشاكل فشل جلب البيانات (Opaque Responses) التي قد تحدث عند استئناف الـ PWA.
